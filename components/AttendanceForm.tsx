...
import { Member, AttendanceStatus, AttendanceRecord, Group, User } from '../types';
...
const AttendanceForm: React.FC<Props> = ({ user, onBack }) => {
  const [activeMembers, setActiveMembers] = useState<Member[]>([]);
  // Inisialisasi: Jika login, pakai kelompok user. Jika tamu, default ke 'Wonokusumo 1'
  const [selectedGroup, setSelectedGroup] = useState<Group>((user?.group || 'Wonokusumo 1') as Group);
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <select 
              className="w-full p-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-emerald-500 font-bold bg-slate-50 focus:bg-white disabled:bg-slate-100 disabled:text-slate-400"
              value={selectedGroup}
              // PERBAIKAN: Tombol hanya mati jika user login DAN bukan Admin. 
              // Jika user null (tamu) atau user adalah Admin, maka tombol aktif.
            ...
            {user !== null && user.role !== 'Admin' && (
              <p className="text-[10px] text-amber-600 font-bold">Wilayah terkunci sesuai akun pengurus.</p>
            )}
          </div>
          <div className="space-y-2">
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
            <h2 className="text-xl font-black text-slate-800 tracking-tight">Daftar Anggota Aktif ({selectedGroup})</h2>
          ...
          <div className="space-y-3">
            ...
            {filteredMembers.length === 0 && (
              <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-slate-200 text-slate-400 flex flex-col items-center gap-4">
                ...
                <div className="font-bold">Peserta tidak ditemukan atau belum terdaftar aktif di kelompok ini.</div>
              </div>
            )}
          </div>
        </div>
